 ...
 *                                          elements.
 * @property {integer} options.throttle     Duration in milliseconds to throttle
 *                                          the scroll event, so that the module
 *                                          scroll callbacks are not called on 
 *                                          each and every browser scroll event.
 * @property {integer} options.duration     The duration of the animation of the
 *                                          transition of the CSS properties
 *                                          that are to be parallaxed.
 ...
 *                                          im terms of space for the parallax
 *                                          effect.
 * @property {integer} options.onAppear     Function callback to call when the
 *                                          watched element appears in the
 *                                          browser viewport.
 * @property {integer} options.onDisappear  Function callback to call when the
 *                                          watched element disappears from the
 *                                          browser viewport.
 * @property {string}  options.enter        Comma separated list of breakpoints
 *                                          in  which the parallax enters, wihch
 *                                          means it is enabled.
...
Responsiville.Parallax.defaults = {
    ...
    //  TODO: 
    // 
    // - Delay iniating the parallax effect.
    // 
    //     by time? ==> NO
    //     by space in pixels? ==> DONE
    //     by space in screen percentages? ==> %
    //     by space in viewport fractions? ==> vh/vw
    //     by watched element visible percentage? ==> %
    // 
    // - Reverse behaviour: sometimes not necessary!
    //
...
Responsiville.Parallax.prototype.offsetTopTotal = function ( element ) {
    ...
    var offsetTop = this.$watched.get( 0 ).offsetTop;
...
Responsiville.Parallax.prototype.scroll = function () {
    ...
    if ( ! watchedElementIsOffViewport ) {
        ...
        if ( ! watchedElementWasInViewport ) {
            ...
            if ( typeof this.options.onAppear == 'string' ) {
                this.options.onAppear != null && window[this.options.onAppear].call( this );
            } else if ( $.isFunction( this.options.onAppear ) ) {
        ...
        if ( watchedElementWasInViewport ) {
            ...
            if ( typeof this.options.onDisappear == 'string' ) {
                this.options.onDisappear != null && window[this.options.onDisappear].call( this );
            } else if ( $.isFunction( this.options.onDisappear ) ) {
    ...
    // Iterate over each parallaxed element.
    ...
    for ( var k=0, length=this.$elements.length; k<length; k++ ) {
        ...
        if ( typeof elementProperties == 'function' ) {
            ...
            $element.
                velocity( 'stop', true ).
                velocity( animationProperties, { duration : this.options.duration } );
